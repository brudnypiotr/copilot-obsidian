...
If you find a security issue in copilot-obsidian, please report it privately rather than opening a public issue.

**Preferred:** GitHub's private reporting at the repository's [Security Advisories](../../security/advisories/new) page.

**Alternative:** Email **brudny.piotr@gmail.com** with subject line `copilot-obsidian security`.
...
This policy covers:
- The plugin code under `skills/`, `agents/`, `scripts/`, `bin/`
- The plugin manifest at `plugin.json` (root)
...
copilot-obsidian assumes a **single-tenant** deployment: one user, one vault, one machine. Several design decisions follow from this assumption and would need explicit hardening for multi-tenant or shared-CI scenarios:
...
- **Skill-internal auto-commit** (inline bash at the end of `save`, `wiki-ingest`, `wiki-fold`, `autoresearch`) runs as the user invoking Copilot CLI. It auto-commits `wiki/`, `.raw/`, and `.vault-meta/` paths to the local repo at the end of each mutating skill. Set `.vault-meta/auto-commit.disabled` (any contents) to opt out per-vault. For shared repos, prefer disabling the commit pattern entirely or using a more restrictive commit policy.
